...
const InterventionView = () => {
  ...
  const getReleatedIntervention = () => {
    const query = selectedLabel === 'unsync' ? 'status == "NOT_SYNCED" AND is_complete == true' : selectedLabel === 'incomplete' ? 'is_complete==false' : selectedLabel === 'all' ? 'intervention_id!=""' : `intervention_key=="${selectedLabel}"`;
