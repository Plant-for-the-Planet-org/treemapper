...
export const isLogin = () => {
  return new Promise((resolve, reject) => {
    Realm.open({
      schema: [Inventory, Species, Polygons, Coordinates, OfflineMaps, User],
    }).then(realm => {
...
export const getAreaName = ({coords}) => {
  return new Promise((resolve, reject) => {
    fetch(
      `https://api.mapbox.com/geocoding/v5/mapbox.places/${coords[0]},${
        coords[1]
      }.json?types=place&access_token=${MAPBOXGL_ACCCESS_TOKEN}`,
    )
      .then(res => res.json())
      .then(res => {
...
export const deleteOfflineMap = ({name}) => {
  return new Promise((resolve, reject) => {
    Realm.open({
      ...
      .then(realm => {
        realm.write(() => {
          const offlineMaps = realm.objectForPrimaryKey(
            'OfflineMaps',
            `${name}`,
          );
      ...
      .then(realm => {
        realm.write(() => {
          ...
          geoJSON.features.map(onePolygon => {
            let onePolygonTemp = {};
            onePolygonTemp.isPolygonComplete =
              onePolygon.properties.isPolygonComplete || false;
            let coordinates = [];
...
export const insertImageAtIndexCoordinate = ({
  inventory_id,
  imageUrl,
  index,
}) => {
    ...
    Realm.open({
      ...
      .then(realm => {
        realm.write(() => {
          ...
          let polygons = Object.values(
            JSON.parse(JSON.stringify(inventory.polygons)),
          );
          let polygonsTemp = [];
          let coordinatesTemp = [];
