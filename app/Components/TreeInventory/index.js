...
const TreeInventory = ({navigation}) => {
      ...
      <FlatList
        ...
        renderItem={({item}) => {
          ...
          let title = item.species
            ? item.species[0]
              ? `${item.species[0].treeCount} ${
                  item.species[0].nameOfTree
                } Tree`
              : '0 Species Tree'
            : '0 Species Tree';
          let data = {
            ...
            date: new Date(Number(item.plantation_date)).toLocaleDateString(),
            imageURL: imageURL,
          };

          return (
            <TouchableOpacity onPress={() => onPressInventory(item)}>
              <InventoryCard icon={item.status == 'pending'} data={data} />
            </TouchableOpacity>
          );
        }}
      />
    ...
    return (
      <View style={styles.cont}>
        {pendingInventory.length > 0 && (
          <>
            <SmallHeader
              leftText={'Pending Upload'}
              rightText={'Upload now'}
              icon={'cloud-upload'}
        ...
        {inCompleteInventory.length > 0 && (
          <>
            <SmallHeader
              ...
              leftText={'Incomplete Registrations'}
              rightText={'Clear All'}
              rightTheme={'red'}
              style={{marginVertical: 15}}
            />
  ...
  const renderLoadingInventoryList = () => {
    return (
      <View style={styles.cont}>
        <Header
          headingText={'Tree Inventory'}
          subHeadingText={
            'It’s empty in here, please register some trees to view them.'
          }
  ...
  const renderEmptyInventoryList = () => {
    return (
      <View style={styles.cont}>
        ...
        />
        <SvgXml
          xml={empty_inventory_banner}
          style={styles.emptyInventoryBanner}
        />
        <View style={styles.parimaryBtnCont}>
          <PrimaryButton
            onPress={() => navigation.navigate('RegisterTree')}
            btnText={'Register Tree'}
          />
        </View>
      </View>
    );
      ...
      <View style={styles.container}>
        <ScrollView showsVerticalScrollIndicator={false}>
          <Header
            headingText={'Tree Inventory'}
            subHeadingText={
              'Inventory will be cleared after upload is complete'
            }
          />
          {renderInventory()}
        </ScrollView>
