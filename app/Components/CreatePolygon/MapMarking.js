...
const infographicText = [
  {
    heading: 'Alrighty!',
    subHeading:
      'Now, please walk to the next corner and tap continue when ready',
  },
  {
    heading: 'Great!',
    subHeading:
...
class MapMarking extends React.Component {
    ...
    getInventory({inventoryID: inventoryID}).then(inventory => {
      ...
      if (inventory.polygons.length > 0) {
        ...
        if (
          ...
          activeMarkerIndex < geoJSON.features[0].geometry.coordinates.length
        ) {
      ...
      try {
        Geolocation.getCurrentPosition(
          position => {
            ...
            geoJSON.features[activePolygonIndex].geometry.coordinates.map(
              oneMarker => {
            ...
            if (!isValidMarkers) {
              alert(
                'You are at the same location. Please walk to the next location.',
              );
  ...
  pushMaker = (complete, currentCoords) => {
    ...
    if (complete) {
      ...
      geoJSON.features[activePolygonIndex].geometry.coordinates.push(
        geoJSON.features[activePolygonIndex].geometry.coordinates[0],
      );
    }
  ...
  render() {
    ...
    let isShowCompletePolygonBtn =
      geoJSON.features[activePolygonIndex].geometry.coordinates.length > 1;
    let coordinatesLenghtShouldBe = geoJSON.features[activePolygonIndex]
      .properties.isPolygonComplete
      ? geoJSON.features[activePolygonIndex].geometry.coordinates.length - 1
      : geoJSON.features[activePolygonIndex].geometry.coordinates.length;
      ...
      <View style={styles.container} fourceInset={{top: 'always'}}>
        ...
        <View>
          ...
          <View style={styles.continueBtnCont}>
            <PrimaryButton
              ...
              btnText={'Select location & Continue'}
        ...
        <LinearGradient
          ...
          <Header
            ...
            subHeadingText={
              'Please visit first corner of the plantation and select your location'
            }
          />
        </LinearGradient>
        <View />
