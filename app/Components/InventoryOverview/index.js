...
const InventoryOverview = ({navigation}) => {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      ...
      let data = {
        ...
        last_screen: 'InventoryOverview',
      };
      updateLastScreen(data);
    });
  }, [initiatState, navigation, state.inventoryID]);

  const initiatState = useCallback(() => {
      ...
      <FlatList
        ...
        renderItem={({item, index}) => {
          return (
            <View>
              <Label
                leftText={`Location Type: ${locationType}`}
                rightText={''}
              />
              <FlatList
                ...
                renderItem={({item: oneCoordinate, index}) => {
                  let normalizeData = {
                    ...
                    measurement: `${oneCoordinate.latitude.toFixed(
                      5,
  ...
  const onPressViewLOC = index => {
    let selectedCoords = Object.values(inventory.polygons[0].coordinates)[
      index
    ];
        ...
        <View style={styles.mainContainer}>
          <View style={styles.screenMargin}>
            <Header
              ...
              closeIcon
              headingText={`Location ${locationTitle}`}
            />
          </View>
          <MapboxGL.MapView
      ...
      if (inventory.polygons.length > 0) {
        let featureList = inventory.polygons.map(onePolygon => {
          return {
            ...
            geometry: {
              type: 'LineString',
              coordinates: Object.values(onePolygon.coordinates).map(
                oneCoordinate => [
        ...
        RNFetchBlob.fs
          ...
          .then(success => {
            alert(
              `GeoJSON file export in ${
                Platform.OS == 'ios' ? 'document' : 'download'
              } directory`,
            );
          })
  ...
  if (inventory) {
    isSingleCoordinate =
      Object.keys(inventory.polygons[0].coordinates).length == 1;
          ...
          <View style={styles.cont}>
            <ScrollView
              ...
              <Header
                ...
                headingText={''}
                subHeadingText={
                  'Trees will be added to your inventory to sync when you have internet.'
                }
              />
              <Label
                leftText={'Plant Date'}
                rightText={new Date(
                  Number(inventory.plantation_date),
                ).toLocaleDateString()}
                onPressRightText={onPressDate}
              />
              {!isSingleCoordinate && (
                <Label leftText={`On Site Registration`} rightText={''} />
              )}
              <Label
                leftText={`Planted Species`}
                rightText={status == 'incomplete' ? 'Edit' : ''}
              ...
              <FlatList
                ...
                renderItem={({item}) => (
                  <Label
                    leftText={`${item.nameOfTree}`}
                    rightText={`${item.treeCount} trees`}
                    style={{marginVertical: 5}}
                    leftTextStyle={{paddingLeft: 20, fontWeight: 'normal'}}
                  />
                )}
              />
              {inventory && inventory.species.length <= 0
