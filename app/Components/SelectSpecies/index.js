...
const SelectSpecies = ({
  ...
  const onPressSpecie = index => {
    ...
    setTimeout(() => {
      speciesList[index].treeCount
        ? setTreeCount(speciesList[index].treeCount)
        : null;
    }, 0);
  };
    ...
    return (
      <TouchableOpacity
        ...
        <View style={{flex: 1}}>
          ...
          {item.treeCount && (
            <Text style={styles.treeCount}>{`${item.treeCount} Trees`}</Text>
          )}
        </View>
      </TouchableOpacity>
    );
  ...
  const renderTreeCountModal = () => {
    let specieName = isShowTreeCountModal
      ? speciesList[activeSpeice].nameOfTree
      : '';
    return (
