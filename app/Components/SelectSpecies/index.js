...
import { View, StyleSheet, Text, Image, FlatList, Modal, TouchableOpacity, KeyboardAvoidingView, TextInput, Platform } from 'react-native';
  ...
  const renderSpeciesCard = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity key={index} onPress={() => onPressSpecie(index)} style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginVertical: 20 }} accessible={true} accessibilityLabel="Species Card" testID="species_card">
        ...
        <View style={{ flex: 1 }}>
          <Text numberOfLines={1} style={styles.speciesLocalName}>{item.localName}</Text>
