...
export class ApprovalBoardService {
  ...
  async getReviewQueue(
    ...
    if (status) {
      ...
      // Default: return all submitted statuses (exclude drafts)
      whereConditions.push(
        inArray(intervention.reviewStatus, [
          'pending', 'resubmitted', 'in_review', 'changes_requested',
  ...
  async submitReviewDecision(
    ...
    return await this.drizzleService.db.transaction(async (tx) => {
      ...
      // Create a review thread if one doesn't exist (e.g., auto-submitted interventions)
      ...
      // Update current thread (only resolve for final decisions, not for 'in_review')
      if (dto.decision !== 'in_review') {
        await tx
          .update(reviewThread)
          .set({
            status: dto.decision === 'changes_requested' ? 'open' : 'resolved',
            resolution: dto.decision as 'approved' | 'changes_requested' | 'rejected',
            resolvedAt: now,
            resolvedById: reviewerId,
            updatedAt: now,
          })
          .where(eq(reviewThread.id, currentThreadId));
      }
  ...
  async checkProjectRequiresApproval(
    projectId: number,
  ): Promise<{ requiresApproval: boolean; requiresInterventionApproval: boolean }> {
    ...
    // Check if approval board is enabled for this project
    const isEnabled = proj.approvalBoardEnabled === true;

    return {
      requiresApproval: isEnabled,
      requiresInterventionApproval: isEnabled,
    };
  }
